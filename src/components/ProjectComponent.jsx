...
function ProjectComponent() {
  ...
  const filteredProjects =
    selectedCategory === "All"
      ? projects
      : projects.filter((project) => project.tag.includes(selectedCategory));

  return (
    <section
      id="projects"
      className="pt-5 pb-3 scroll-mt-16 section-area"
      style={{ fontFamily: "SpaceGrotesk", fontSize: "1rem" }}
    >
      ...
      <div className="grid mt-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {(showAll ? filteredProjects : filteredProjects.slice(0, 3)).map(
          (project, index) => (
            <a
              key={index}
              href={project.link}
            ...
            >
              <div className="card h-full overflow-hidden">
                <h2 className="font-bold text-[#b859eb] clamp-1">
                  {project.title}
                </h2>
                <h2 className="font-extralight text-gray-200 clamp-1">
                  {project.tag.join(" | ")}
                </h2>
                ...
                <div className="flex flex-row justify-start items-center font-extralight text-gray-400 pb-3 clamp-1">
                  {project.sub_tag.map((subtag, subIndex) => (
                    <p key={subIndex}>
                      {subtag}
                      {subIndex < project.sub_tag.length - 1 && (
                        <span>&nbsp;|&nbsp;</span>
                      )}
                    </p>
                  ))}
      ...
      {filteredProjects.length > 3 && (
        <button
          className="project-button mt-5"
          onClick={() => setShowAll(!showAll)}
        >
          <h2 className="font-bold">{showAll ? "Show Less" : "Show more"}</h2>
        </button>
      )}
    </section>
  );
