const subarrays = [
  ["Kotlin", "Jetpack Compose"], 
  ["MERN", "React", "MongoDB"], 
  ["HTML", "CSS", "Flask"], 
  ["React", "TailwindCSS", "Javascript"],
  ["Python", "scikit-learn", "Transformers"],
  ["Flutter", "Dart", "Firebase"],
  ["React", "Python", "FastAPI", "Supabase"]
]

const projects = [
  {
    order: "1",
    title: "Chess Arena",
    description: "A simple Chess game developed in Flutter with Singleplayer and Multiplayer support.",
    long_description: "Chess Arena is a modern chess app built with Flutter for Android and Web. Play against a smart AI with multiple difficulty levels or challenge friends in real-time multiplayer. Create private rooms using a 6-digit code or join instantly, with smooth gameplay and synchronized moves for an engaging chess experience.",
    link: "https://github.com/Govind-Sankar/chessarena",
    image: "",
    color: "#6695A9",
    tag: ["Android Development"],
    sub_tag: subarrays[5],
  },
  {
    order: "2",
    title: "IMDb Sentiment Analyzer",
    description: "An ML project for sentiment analysis on the IMDb movie reviews dataset.",
    long_description: "A machine learning project for sentiment analysis on the IMDb movie reviews dataset. This project compares classical ML models (Logistic Regression, SVM, Random Forest) with a fine-tuned DistilBERT transformer model. The web interface is built with Streamlit and hosted on HuggingFace Spaces.",
    link: "https://github.com/Govind-Sankar/IMDb-Sentiment-Analyzer",
    image: "",
    color: "#B28CB2",
    tag: ["Machine Learning"],
    sub_tag: subarrays[4],
  },
  {
    order: "3",
    title: "Nexus Evaluator",
    description: "An automated assignment evaluation system.",
    long_description: "A responsive MERN-based To-Do Web Application featuring secure JWT authentication, CRUD operations, task filtering, and priority management. It offers light/dark mode, timestamped tasks, and a modern UI with Tailwind CSS. It ensures seamless performance with MongoDB Atlas for real-time task management.",
    link: "https://github.com/Govind-Sankar/Nexus-Evaluator",
    image: "",
    color: "#6695A9",
    tag: ["Web Development", "Machine Learning"],
    sub_tag: subarrays[6],
  },
  {
    order: "4",
    title: "Impensa",
    description: "An Expense Tracker App built for Android with Kotlin and Jetpack Compose",
    long_description: "Impensa is a modern Android expense tracker built with Kotlin and Jetpack Compose. It offers a clean, colorful UI for adding, editing, or deleting expenses, complete with categories and dates. Visualize monthly spending through interactive pie charts, enjoy dark mode, and get seamless in-app updates for a smooth experience.",
    link: "https://github.com/Govind-Sankar/Impensa",
    image: "",
    color: "#6695A9",
    tag: ["Android Development"],
    sub_tag: subarrays[0],
  },
  {
    order: "5",
    title: "Snake Game",
    description: "A snake game built for Android with Kotlin and Jetpack Compose",
    long_description: "A fun and lightweight Snake Game built for Android using Kotlin and Jetpack Compose. Control the snake, eat food, and grow longer while avoiding collisions. Simple to play, optimized for smooth performance, and perfect for casual gaming.",
    link: "https://github.com/Govind-Sankar/Snake-Game",
    image: "",
    color: "#6695A9",
    tag: ["Android Development"],
    sub_tag: subarrays[0],
  },
  {
    order: "6",
    title: "To-Do WebApp",
    description: "A simple To-Do Web application made with MongoDB and React.",
    long_description: "A responsive MERN-based To-Do Web Application featuring secure JWT authentication, CRUD operations, task filtering, and priority management. It offers light/dark mode, timestamped tasks, and a modern UI with Tailwind CSS. It ensures seamless performance with MongoDB Atlas for real-time task management.",
    link: "https://github.com/Govind-Sankar/To-Do-WebApp",
    image: "",
    color: "#6695A9",
    tag: ["Web Development"],
    sub_tag: subarrays[1],
  },
  {
    order: "7",
    title: "Note Keeper WebApp",
    description: "A web application designed to securely store and manage user notes.",
    long_description: "Note Keeper is a web-based application designed to securely store and manage user notes. Built using Python (Flask) for the backend and HTML/CSS for the frontend. It offers User Authentication, Notes Management and Secure Data Handling.",
    link: "https://github.com/Govind-Sankar/Note-Keeper-WebApp",
    image: "",
    color: "#B28CB2",
    tag: ["Web Development"],
    sub_tag: subarrays[2],
  },
  {
    order: "8",
    title: "Flash-Light App",
    description: "A simple Flashlight (Torch) App made with Kotlin and Jetpack Compose.",
    long_description: "Flashlight-App is a lightweight and minimalist Android app built with Kotlin and Jetpack Compose. It provides a simple way to toggle your device's flashlight on or off with a clean, modern UI. Easy to install, open-source, and MIT licensed—perfect for users and developers alike.",
    link: "https://github.com/Govind-Sankar/Flashlight-App",
    image: "",
    color: "#B28CB2",
    tag: ["Android Development"],
    sub_tag: subarrays[0],
  },
  {
    order: "9",
    title: "Personal Portfolio Website",
    description: "A modern personal portfolio website made with React and Tailwind.",
    long_description: "A personal portfolio showcasing skills. Features interactive project displays, certifications, and a contact form with CAPTCHA protection. Built with React, TailwindCSS, and modern web practices, the site highlights professional growth, creativity, and technical expertise in a sleek, user-friendly design.",
    link: "https://github.com/Govind-Sankar/Portfolio-Website",
    image: "",
    color: "#B28CB2",
    tag: ["Web Development"],
    sub_tag: subarrays[3],
  },
];

export default projects;